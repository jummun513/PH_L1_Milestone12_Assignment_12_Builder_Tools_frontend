...
const CheckoutPage = () => {
    ...
    const { mutateAsync, data: paymentOrderData, isSuccess } = usePaymentOrder();

    useEffect(() => {
        if (isSuccess) {
            window.location.replace(paymentOrderData.data.url);
        }
    }, [isSuccess, paymentOrderData]);
            ...
            <div className="w-full mx-auto shadow-lg max-w-5xl py-10 px-5 rounded-md">
                <div className="mb-10">
                    ...
                    {data?.isPaid && <div>
                        ...
                        <p className="text-gray-800 font-semibold md:mt-1">Transaction Id: <span className="text-red-500">{data?.transactionId}</span></p></div>}
                </div>
                ...
                {
                    data?.isPaid ?
                        <p className="text-end mt-5 md:mt-8"><button className="btn btn-xs sm:btn-sm xl:btn-md bg-primary hover:bg-secondary border-none text-gray-50">ReOrder</button></p>
                        :
